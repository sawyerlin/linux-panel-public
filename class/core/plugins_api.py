...
from libs.remote_plugins import (
    get_plugins,
    get_plugin_info,
...
class plugins_api:
    ...
    def menuApi(self):
        ...
        if os.path.exists(menu_file):
            ...
            for menu_data in tlist:
                if tag == 'mysql':
                    if 'mysql-yum' == menu_data['name']:
                        tag = 'mysql-yum'
                    elif 'mysql-apt' == menu_data['name']:
                        tag = 'mysql-apt'
        ...
        if not tag_found:
            if tag == 'op_waf':
                content = mw.readFile('data/plugin_opwaf_notfound.html')
            elif tag == 'mysql':
                os_name = mw.getOsName()
                if os_name == 'centos':
                    content = mw.readFile('data/plugin_mysql_yum_notfound.html')
                else:
                    content = mw.readFile('data/plugin_mysql_apt_notfound.html')
            elif tag == 'pureftp':
                content = mw.readFile('data/plugin_pureftp_notfound.html')
    ...
    def installApi(self):
        ...
        plugin_info = get_plugin_info(name, mw.getOsName())
        self.hookInstall(plugin_info)

        # TODO (DA): build dependencies tree
        depencencies_download_command = ''
        if name in ('mysql', 'mongodb'):
            depencencies_download_command = (
                ' && python download_plugin.py ' +
                get_plugin_remote_url('php') +
                '/ ' +
                mw.getPluginDir() +
                '/php'
            )
        plugin_folder = mw.getPluginDir() + '/' + name
        download_command = (
            'cd ' + mw.getToolDir() +
            ' && source ../bin/activate' +
            depencencies_download_command +
            ' && python download_plugin.py ' + get_plugin_remote_url(name) +
            '/ ' + plugin_folder
        )

        install_command = 'cd ' + mw.getPluginDir() + '/' + name + ' && '
        shell = plugin_info['shell']

        if plugin_info.get("install_pre_inspection", None):
            install_command += f'bash {shell} install_pre_inspection {version} && '

        install_command += 'cd ' + mw.getPluginDir() + '/' + name + ' && bash ' + \
            shell + ' install ' + version
        ...
        mw.M('tasks').delete(id=7)
        mw.M('tasks').add(
            'name,type,status,addtime, execstr',
            task_add)
    ...
    def uninstallApi(self):
        ...
        uninstall_command = "cd " + mw.getPluginDir() + "/" + \
            name + " && /bin/bash " + pluginInfo["shell"] \
            + " uninstall " + version

        data = mw.execShell(uninstall_command)
        if mw.isDebugMode():
    ...
    def getAllListPage(self, sType='0', kw='', page=1, pageSize=10):
        plugins_info = []
        os_name = mw.getOsName()
        for data in get_plugins(os_name):
    ...
    def run(self, name, func, version='', args='', script='index'):
        if name == 'mysql':
            installed_path = mw.getServerDir() + name
            if not os.path.exists(installed_path):
                installed_path = mw.getServerDir() + "/mysql-apt"
                if not os.path.exists(installed_path):
                    path = self.__plugin_dir + '/mysql-yum/' + script + '.py'
                else:
                    path = self.__plugin_dir + '/mysql-apt/' + script + '.py'
        else:
